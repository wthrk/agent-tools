...
pub fn status(profile_name: &str) -> Result<()> {
    ...
    if is_serverless(&config) {
        ...
        if let Ok(details) = run_runpodctl_json(&[
            "-o",
            "json",
            "serverless",
            "get",
            ...
            if let Some(summary) = summarize_worker_states(&details) {
                println!("{} {}", "Worker states:".green(), summary.cyan());
            }
            if let Some(last) = latest_worker_event(&details) {
                println!("{} {}", "Latest worker event:".green(), last.cyan());
            }
        }
...
fn build_create_args(config: &RunpodConfig) -> Result<Vec<String>> {
    ...
    if let Some(value) = &config.template_id {
    ...
    if let Some(value) = &config.gpu_id {
        args.push("--gpu-id".to_string());
        args.push(value.clone());
    }
    if let Some(value) = &config.cloud_type {
        args.push("--cloud-type".to_string());
        args.push(value.clone());
    }
    if let Some(value) = &config.compute_type {
        args.push("--compute-type".to_string());
        args.push(value.clone());
    }
    if let Some(value) = config.gpu_count {
        args.push("--gpu-count".to_string());
        args.push(value.to_string());
    }
    if let Some(value) = config.container_disk_in_gb {
        args.push("--container-disk-in-gb".to_string());
        args.push(value.to_string());
    }
    if let Some(value) = config.volume_in_gb {
        args.push("--volume-in-gb".to_string());
        args.push(value.to_string());
    }
    if let Some(value) = &config.volume_mount_path {
        args.push("--volume-mount-path".to_string());
        args.push(value.clone());
    }
    if let Some(value) = &config.ports
        && !value.is_empty()
    {
        args.push("--ports".to_string());
        args.push(value.join(","));
    }
    if let Some(value) = &config.data_center_ids
        && !value.is_empty()
    {
        args.push("--data-center-ids".to_string());
        args.push(value.join(","));
    }
    if let Some(value) = &config.env
        && !value.is_empty()
    {
        args.push("--env".to_string());
        args.push(env_to_json(value)?);
    }
...
fn build_serverless_create_args(config: &RunpodConfig) -> Result<Vec<String>> {
    ...
    if let Some(value) = config.workers_min {
        args.push("--workers-min".to_string());
        args.push(value.to_string());
    }
    if let Some(value) = config.workers_max {
...
fn run_runpodctl_json(args: &[&str]) -> Result<Value> {
    let owned = args
        ...
        .collect::<Vec<_>>();
    run_runpodctl_json_owned(&owned, "runpodctl command")
}
...
fn extract_pod_id(value: &Value) -> Option<String> {
    value
        ...
        .or_else(|| value.get("podId").and_then(Value::as_str))
        .or_else(|| value.get("pod_id").and_then(Value::as_str))
        .or_else(|| {
            value
                .get("data")
...
fn extract_endpoint_id_from_list_by_name(value: &Value, name: &str) -> Option<String> {
    if let Some(array) = value.as_array() {
        return extract_endpoint_id_from_items(array, name);
    }
...
fn extract_workers(item: &Value) -> Option<String> {
    ...
    match (current, min, max) {
        (Some(current), Some(min), Some(max)) => Some(format!("{current} (min={min}, max={max})")),
        (Some(current), _, _) => Some(current.to_string()),
        _ => None,
    }
}

fn summarize_worker_states(value: &Value) -> Option<String> {
    let workers = value.get("workers").and_then(Value::as_array)?;
    if workers.is_empty() {
        return Some("none".to_string());
    }
    let mut running = 0_u64;
    let mut exited = 0_u64;
    let mut other = 0_u64;
    for worker in workers {
        let state = worker
            .get("desiredStatus")
            .or_else(|| worker.get("status"))
            .and_then(Value::as_str)
            .unwrap_or("UNKNOWN");
    ...
    Some(format!(
        "running={running}, exited={exited}, other={other}, total={}",
        workers.len()
    ))
}

fn latest_worker_event(value: &Value) -> Option<String> {
    let workers = value.get("workers").and_then(Value::as_array)?;
    let worker = workers.last()?;
...
fn is_serverless(config: &RunpodConfig) -> bool {
    config
        .deployment
        .as_deref()
        .is_some_and(|value| value.eq_ignore_ascii_case("serverless"))
}
        ...
        .and_then(TomlValue::as_table_mut)
    {
        ...
        if let Some(provider_table) = provider.as_table_mut() {
            provider_table.insert(
                "name".to_string(),
                TomlValue::String("RunPod Serverless".to_string()),
            );
            provider_table.insert(
                "base_url".to_string(),
            ...
            provider_table.insert(
                "env_key".to_string(),
                TomlValue::String("RUNPOD_API_KEY".to_string()),
            );
            provider_table.insert(
                "wire_api".to_string(),
...
mod tests {
    ...
    fn build_create_args_includes_required_and_optional() {
        ...
        assert!(args.iter().any(|value| value == "--template-id"));
        assert!(args.iter().any(|value| value == "tpl-1"));
        assert!(args.iter().any(|value| value == "--gpu-id"));
    }
    ...
    fn build_serverless_args_includes_template_and_workers() {
        ...
        assert!(args.iter().any(|value| value == "serverless"));
        assert!(args.iter().any(|value| value == "--template-id"));
        assert!(args.iter().any(|value| value == "tpl-sls"));
    }
