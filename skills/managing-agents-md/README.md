...
**事実:**
- AGENTS.mdはプロジェクトルートに配置
- サブディレクトリにネスト可能（最も近いファイルが優先）
- 標準Markdown、必須フィールドなし
- 複数のAIエージェント（Cursor、GitHub Copilot、Claude Code等）が同じファイルを参照

**ルール:**
- 既存ファイルを上書きする前に必ず確認
- 既存の内容を尊重し、必要なセクションのみ更新
- コマンドはコードブロック内に実行可能な形式で記述
- 150行以下を目標
- コマンドを早い段階に配置（フラグ付き）
- 説明よりコード例を優先

**警告:**
- 既存のAGENTS.mdを上書きするとデータが失われる可能性あり
- シークレットや認証情報を含めない
- 重複を避け、外部リソースへリンク

## ヒント

- **検出優先順位:** package.json > Cargo.toml > pyproject.toml > go.mod
- **一般的なスタックパターン:**
  - Node.js: package.jsonの`scripts`を確認
  - Rust: Cargo.tomlの`[[bin]]`や`[workspace]`を確認
  - Python: `[tool.pytest]`や`[tool.poetry]`を確認
- **モノレポサポート:** パッケージ固有の指示はサブディレクトリにネストしたAGENTS.mdを作成
